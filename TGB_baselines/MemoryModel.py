...
from TGB_baselines.utils.utils import NeighborSampler
from TGB_baselines.modules import TimeEncoder, MergeLayer, MultiHeadAttention

class MemoryModel(torch.nn.Module):

    def __init__(self, node_raw_features_dim: int, edge_raw_features_dim: int, num_nodes: int,
        ...
        # set this for each sequence
        self.node_raw_features = None
        self.edge_raw_features = None
        self.neighbor_sampler = None
        # self.src_node_mean_time_shift = None
        # self.src_node_std_time_shift = None
        # self.dst_node_mean_time_shift_dst = None
        # self.dst_node_std_time_shift = None

        self.node_feat_dim = node_raw_features_dim
        self.edge_feat_dim = edge_raw_features_dim
        ...
        if self.model_name == 'JODIE':
            ...
            self.embedding_module = GraphAttentionEmbedding(
                # set sampler and edge feature (dummy) for each sequence
    ...
    def set_feature(self, node_raw_features: torch.Tensor, edge_raw_features: torch.Tensor):
        assert self.model_name in ['TGN', 'DyRep'], f'code has modified for {self.model_name}!'
        self.node_raw_features = node_raw_features
        self.edge_raw_features = edge_raw_features
        self.embedding_module.node_raw_features = node_raw_features
        self.embedding_module.edge_raw_features = edge_raw_features
