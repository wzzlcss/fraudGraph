...
path = osp.join(os.getcwd(), 'data', 'JODIE')
...
def train():
    ...
    for batch in train_loader:
        ...
        # Move message stores if they already exist
        for store in [memory.msg_s_store, memory.msg_d_store]:
            for key in list(store.keys()):
                store[key] = [m.to(device) for m in store[key]]
