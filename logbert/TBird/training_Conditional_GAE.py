...
data_path='../output/tbird/train_with_time'
vocab_path='../output/tbird/vocab.pkl'
...
train_set = LogGraphDatasetAdjPair(train_iter)
...
valid_set = LogGraphDatasetAdjPair(valid_iter)
...
for it in range(3):
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        if step % accum_steps == 0:
            ...
            print(f"Epoch: {it}; Total avg Loss ({step} / {n_train_sample}): {running_loss}")
            running_loss = 0.0
            #######
        if step % valid_step == 0:
            ...
            print(f"Epoch: {it} Step: {step}; validation roc_auc (true condition): {auc_cond}; (random input): {auc_rand_cond}; (random input & output): {auc_rand}")
            model.train()
