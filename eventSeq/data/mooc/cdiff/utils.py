...
def run_eval(args):
    ...
    print('distance (w/o filter) mean is {:.3f} and std is {:.3f}'.format(
        distances_wo_filter.mean(), distances_wo_filter.mean(-1).std())
    )

    rmse_types_wo_filter = np.array(rmse_types_wo_filter)
    print('rmse type (w/o filter) mean is {:.3f} and std is {:.3f}'.format(
        rmse_types_wo_filter.mean(), rmse_types_wo_filter.std())
    )

    distances_with_filter = np.array(distances_with_filter)
    print('distance (with filter) mean is {:.3f} and std is {:.3f}'.format(
        distances_with_filter.mean(), distances_with_filter.mean(-1).std())
    )

    rmse_types_with_filter = np.array(rmse_types_with_filter)
    print('rmse type (with filter) mean is {:.3f} and std is {:.3f}'.format(
        rmse_types_with_filter.mean(), rmse_types_with_filter.std())
    )

    print('rmse # of events is {: .3f}'.format(rmse_num_events))
    print('mae # of events is {: .3f}'.format(mae_num_events))

    print('rmse time mean is {:.3f} and std is {:.3f}'.format(rmse_mean, rmse_std))
    print('mape time mean is {:.3f} and std is {:.3f}'.format(mape_mean, mape_std))
    print('smape time mean is {:.3f} and std is {:.3f}'.format(smape_mean, smape_std))
    ...
    with open(path_samples_result, 'w') as f:
        f.write('distance (w/o filter) mean: {:.3f} and std: {:.3f}\n'.format(
            distances_wo_filter.mean(), distances_wo_filter.mean(-1).std())
        )

        f.write('rmse type (w/o filter) mean: {:.3f} and std: {:.3f}\n'.format(
            rmse_types_wo_filter.mean(), rmse_types_wo_filter.std())
        )

        f.write('distance (with filter) mean: {:.3f} and std: {:.3f}\n'.format(
            distances_with_filter.mean(), distances_with_filter.mean(-1).std())
        )

        f.write('rmse type (with filter) mean: {:.3f} and std: {:.3f}\n'.format(
            rmse_types_with_filter.mean(), rmse_types_with_filter.std())
        )
        ...
        f.write('rmse time mean: {:.3f} and std: {:.3f}\n'.format(rmse_mean, rmse_std))
        f.write('mape time mean: {:.3f} and std: {:.3f}\n'.format(mape_mean, mape_std))
        f.write('smape time mean: {:.3f} and std: {:.3f}\n'.format(smape_mean, smape_std))
    ...
    # plt_marginal_dt_dist(args, pred_x_clone[:, :, 0].unsqueeze(-1), gt_x.unsqueeze(-1), mode='diff')
    ...
    num_seqs_analysis = args.num_seqs_analysis

    ######################################################################################################
    ##################################### Specify Figure Saving Path #####################################
    ######################################################################################################

    # path_seq_analysis_saving = os.path.join(args.path_samples, 'seqs_analysis')
    # if not os.path.exists(os.path.dirname(path_seq_analysis_saving)):
    #     os.mkdir(os.path.dirname(path_seq_analysis_saving))

    args.path_seq_analysis_saving = args.path_samples

    # num_seqs, colors, pred_x, pred_e, gt_x, gt_e, time_range, save_path
    # pred_x = torch.cumsum(pred_x, dim=1)
    # # gt_x = torch.cumsum(gt_x, dim=1)
    # plt_seqs_analysis(args, num_seqs_analysis, colors, pred_x_clone[:, :, 1], pred_e_clone[:, :, 1], gt_x, gt_e,
    #                   args.time_range, mode='diff')

    # filter = True
    # distance_del_cost_linear = np.linspace(0.1, 20, 100, endpoint=False)
    # distances_with_filter = get_distances_diffusion(pred_x, pred_e, gt_x, gt_e, args.num_classes, filter,
    #                                                 args.time_range, distance_del_cost_linear, trans_cost)

    # plt_diff_del_cost_distances(distances_with_filter, distance_del_cost_linear, args)
