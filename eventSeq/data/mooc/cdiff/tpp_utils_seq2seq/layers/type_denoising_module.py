...
class TypeDenoisingModule(nn.Module):
    ...
    def forward(self, t, e, x, hist, non_padding_mask):
        ...
        :param x: dt and type seq representation, B x L_tgt x 2*Dim
        :param t:
        :param h:
        ...
        order = self.temporal_enc(order.to(x.device))
        time_embed = t.view(x.size(0), 1, int(self.transformer_dim/4))
        t = torch.cat([time_embed]*x.size(1), dim=1) + order
        e = self.event_emb(e) + order
        x = self.temporal_enc(x) + order

        tgt = torch.cat([x,x,e,t], dim=-1)
